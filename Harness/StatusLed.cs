...
internal sealed class StatusLed : IDisposable
{
    ...
    public StatusLed(GpioController gpioController, IGamepadAvailable gamepadAvailable, ILogger<StatusLed> logger)
    {
        ...
        gamepadAvailable.AvailableChanged += (s, e) => SetRedLed(!e.Value);
        SetRedLed(!gamepadAvailable.IsAvailable);
    }
